...
- **No tables.** In a terminal a markdown table becomes box-drawing/pipe noise with no header association. Give tabular data as prose that repeats the column label with each value ("Plan A is $10/mo with 5 seats; Plan B is $20 with 20"). A real table is fine *only* on a rendered web/desktop surface.
